...
def get_news():
    url = 'https://sports.news.naver.com/ranking/index.nhn?type=popular'
    news_df = pd.DataFrame(columns=("Title","Article"))
    idx = 0

    search_url = urllib.request.urlopen(url).read()
    soup = BeautifulSoup(search_url,'html.parser')
    content = soup.find('div',{'class':'aside_rank_news'})
    ul = content.find('ul')
    links = ul.find_all('li')
    for link in links:
        news_url = "https://sports.news.naver.com" + link.find('a').get('href')
        news_link = urllib.request.urlopen(news_url).read()
        news_html = BeautifulSoup(news_link, 'html.parser')

        date = news_html.find('div',{'class','info'}).get_text()
        date = date.replace(".","")
        date = date[5:14]
...
def wordcloud(date, top_keyword):
    icon = Image.open('/mnt/crawling/images/sports.png')
    ...
    wc.to_file('/mnt/result/Sports/{}-W.png'.format(date))
...
def showGraph(date, top_keyword):
    ...
    plt.savefig('/mnt/result/Sports/{}-G.png'.format(date))
...
top_10.to_csv('/mnt/result/Sports/{}-C.csv'.format(date),index=False,header=False, encoding='utf-8-sig')
